...
const WrongWordsManager = {
    ...
    toggleExample(cardElement) {
        const exampleContainer = cardElement.querySelector('[data-field="exampleContainer"]');
        if (exampleContainer) {
            const isHidden = exampleContainer.classList.contains('hidden');
            if (isHidden) {
                // Only show if there is actual example text
                const exampleText = exampleContainer.querySelector('[data-field="example"]');
                if (exampleText && exampleText.textContent.trim()) {
